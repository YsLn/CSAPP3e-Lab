...
#define ARG_NUM 32
    ...
    char **argv = (char **)malloc(ARG_NUM * sizeof(char *));
    int bg = parseline(cmdline, argv);  

    if (NULL == argv[0]) return;

    // buildin cmd -> quit, jobs, fg, bg 
...
int builtin_cmd(char **argv) 
{
  if (0 == strncmp(argv[0], "jobs", 5)) {
      listjobs(jobs);
      return 1;
  }
  if (0 == strncmp(argv[0], "quit", 5)) {
      exit(0);
  }
  if (0 == strncmp(argv[0], "fg", 3) ||
      0 == strncmp(argv[0], "bg", 3)) {
      do_bgfg(argv);
      return 1;
  }
...
void do_bgfg(char **argv) 
{
    ...
    int job_flag = 0;    // is jobid?

    char *str = argv[1];
    if ('%' == str[0]) {  // yes, is jobid
        job_flag = 1;
        str++;
    }
    ...
    if (0 == strncmp(argv[0], "fg", 3)) {
        assert(fgpid(jobs) == 0);
        j->state = FG;
        assert(fgpid(jobs) == j->pid);
        return waitfg(j->pid);
    }

    // it is `bg`
    assert(j->state != FG && j->state != UNDEF);
    j->state = BG;
    kill(-1 * j->pid, SIGCONT);

    fprintf(stderr, "[%d] (%d) %s", j->jid, j->pid, j->cmdline);
    return;
}
...
void sigchld_handler(int sig) 
{
    ...
    while ((pid = waitpid(-1, &status, WUNTRACED | WNOHANG)) > 0) {
        struct job_t *j = getjobpid(jobs, pid);
        assert(j != NULL);
        assert(j->state != UNDEF);
        
        if (WIFSTOPPED(status)) {    // child stop
            j->state = ST;
